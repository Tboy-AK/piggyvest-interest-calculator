/**
 * @description The function calculates the interest on a daily basis.
 * Therefore, time is equivalent to 1 day.
 * @param {Number} principal The base and incremental sum
 * @returns {Number} The daily simple interest
 */
...
const piggyvestDailyInterestResult = () => {
  const principal = Number(document.querySelector('.piggyvest-daily-interest input.principal').value);
  const interval = Number(document.querySelector('.piggyvest-daily-interest input.interval').value);
  const period = Number(document.querySelector('.piggyvest-daily-interest input.period').value);

  document.querySelector('.piggyvest-daily-interest p.result').innerText = `=N= ${calculateTotalInterest(principal, interval, period)}`;
}

(() => {
  const principalInput = document.querySelector('.piggyvest-daily-interest input.principal');
  const intervalInput = document.querySelector('.piggyvest-daily-interest input.interval');
  const periodInput = document.querySelector('.piggyvest-daily-interest input.period');
